module.exports = app => {
    ...
    const save = (req, res) => {
        ...
        try {
            existsOrError(category.name, 'Nome da categoría nao informado');
            lesOrError(category.name, 'Nome da categoria Demasiado Largo');
        } catch (msg) {
    ...
    const remove = async(req, res) => {
        try {
            existsOrError(req.params.id, 'Codigo da categoria nao foi informado.');
    ...
    const getTreeWithPath = (req, res) => {
        app.db('categories')
            .then(categories => res.json(toTree(withPath(categories))))
            .catch(err => res.status(500).send(err))
    }

    return {get, getById, save, remove, getTree, getTreeWithPath };
